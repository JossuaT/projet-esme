...
processed_df = processed_df.withColumn(
    "heat_index",
    processed_df.temperature + (0.5555 * ((6.11 * (10 ** ((7.5 * processed_df.temperature) / (237.7 + processed_df.temperature))) * (processed_df.humidity / 100)) - 10)))
...
processed_df = processed_df.withColumn(
    'time_of_day',
    when(hour(processed_df['date'])>=16, "Soirée")
        .when(hour(processed_df['date'])>=12, "Après-midi")
            .when(hour(processed_df['date'])>=6, "Matin").otherwise("Nuit")
    )
...
query = (kafka_output_df
    ...
    .option("topic", "projet-esme-final")                   # Nom du topic Kafka de destination
